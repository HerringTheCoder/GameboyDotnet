...
public static class TestHelper
{
    ...
    public static IDictionary<string, List<CpuTest>> TestCases(int skip, int take, string? fileName = null)
    {
        ...
        var fileQuery = files.Skip(skip).Take(take).Where(x => fileName is null || x.Name.Equals(fileName, StringComparison.InvariantCultureIgnoreCase));

        foreach (var file in fileQuery)
        {
            using var stream = file.OpenRead();

            allTests.Add(file.Name, JsonSerializer.Deserialize<List<CpuTest>>(stream, DefaultJsonOptions)!);
        }
