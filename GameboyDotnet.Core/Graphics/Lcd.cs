...
public class Lcd(MemoryController memoryController)
{
    ...
    public byte Lyc => memoryController.ReadByte(Constants.LYCompareRegister);

    public byte Ly
    {
        get => memoryController.ReadByte(Constants.LYRegister);
        set => memoryController.WriteByte(Constants.LYRegister, value);
    }

    public byte Stat
    {
        get => memoryController.ReadByte(Constants.LcdStatusRegister);
        set => memoryController.WriteByte(Constants.LcdStatusRegister, value);
    }
    ...
    {
        Stat = (byte)((Stat & 0b11111100) | (byte)currentPpuMode);
        
        if (currentPpuMode != PpuMode.VramAccessMode3)
        {
